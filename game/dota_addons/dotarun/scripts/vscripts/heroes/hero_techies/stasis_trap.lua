...
function StasisTrapPlant(caster, target_point, ability)
    local ability_level = ability:GetLevel() - 1

    -- Modifiers
    local modifier_tracker = "modifier_stasis_trap_tracker_datadriven"
    local modifier_stasis_trap_invisibility = "modifier_stasis_trap_invisibility_datadriven"
    local modifier_stasis_trap = "modifier_stasis_trap_datadriven"
    ...
    local sign = CreateUnitByName("custom_sign", target_point, false, nil, nil, caster:GetTeamNumber())
    sign:AddNewModifier(caster, ability, "modifier_kill", {Duration = duration})
    sign:SetAngles(0, -90, 0)
